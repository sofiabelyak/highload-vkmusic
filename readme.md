...
|------------------------|------------------|----------------------|
| **ID трека**           | 16 Б             | 1.6 КБ               |
| **Текстовые данные**   | 400 Б (400 символов в среднем * 1 байт)            | 40 КБ                |
| **Обложка**            | 80 КБ            | 8 МБ                 |
| **Теги**               | 100 Б            | 10 КБ                |
| **Дата добавления**    | 8 Б            | 0.8 КБ               |
| **Дополнительные данные** (сонграйтеры, лейбл, рейтинги) | 19.5 КБ = 100 КБ - (16 Б + 400 Б + 80 КБ + 100 Б + 8 Б) | 1.95 МБ = 10 МБ - (1.6 КБ + 40 КБ + 8 МБ + 10 КБ + 0.8 КБ) |
...
| API-запросы       | 0.06       | `7.2k RPS × 1 КБ/запрос × 8 бит`        |
...
- **7.2k RPS**: Пиковые запросы к API (поиск, плейлисты, аутентификация).

#### Суточный трафик (ТБ/день)
| Тип трафика       | Значение | Формула                                |
|--------------------|----------|----------------------------------------|
| Аудио             | 440     | `10M DAU × 12 треков × 3.66 МБ`           |
| API               | 3.11        | `311,43M запросов × 10 КБ`                |
...
- **Управление плейлистами (1.43М/день)**:  
...
| **Управление плейлистами** | 1.4M               | 48.47% — Россия, 15.09% — Беларусь, 14.18% — Казахстан, 8.68% — Молдова, 13.58% — Резервные ДЦ |
...
| **Управление плейлистами** | 0.42M/день | 0.14М/день          | 0.118M/день         | 0.222M/день    | 0.198M/день     | 0.121M/день     | 0.19M/день           |
...
Общая нагрузка:
Для 7.2k RPS потребуется примерно 7.2 секунды CPU времени на обработку SSL-рукопожатий.
...
| **search_queries** | 16 (id) + 16 (user_id) + 100 (query) + 4 (created_at) | 136 байт      | 50 млн = 10M DAU × 5 запросов/день     | ~6.35 ГБ  в день     |
...
![Физическая схема БД](img/schm6.png)
...
### Входящие потоки
1. **Клиентские запросы** поступают от:
   - Веб-клиентов (Web Client)
   - Мобильных приложений (iOS/Android)
   - CDN (для статического контента)

2. **Маршрутизация**:
   - Все запросы сначала попадают на HAProxy Load Balancer
   - Балансировщик распределяет нагрузку на API Gateway

### Обработка запросов
1. **API Gateway**:
   - Маршрутизирует запросы к соответствующим микросервисам
   - Обрабатывает аутентификацию (через Auth Service)
=
2. **Микросервисы**:
   - Каждый сервис обрабатывает свой тип запросов:
     - Auth Service: аутентификация/авторизация
     - Search Service: поисковые запросы
     - Playlist Service: управление плейлистами
     - Streaming Service: стриминг контента
     - Recommendation Service: сервис для рекоммендаций
     - Analytics Service: аналитика данных

### Работа с данными
1. **Базы данных**:
   - PostgreSQL: для структурированных данных (Auth, Playlists)
   - ElasticSearch: для поисковых индексов
   - Cassandra: для потоковых данных 
   - Redis: кэширование и сессии

2. **Асинхронные события**:
   - Все сервисы отправляют события в Kafka
   - Analytics Service обрабатывает события и сохраняет в ClickHouse
...
1. **HAProxy**:
   - Распределяет нагрузку между инстансами API Gateway

2. **CDN**:
   - Кэширует статический контент
   - Снижает нагрузку на основные сервисы

### Внутренняя балансировка
1. **Kubernetes**:
   - Обеспечивает балансировку между подами микросервисов

2. **NGINX**:
   - Маршрутизация на уровне L7
   - Поддержка gRPC 
...
| Сфера     |   Меры    |
| ------    | -----     |
| Ранг ДЦ   | Tier III/IV (в зависимости от конкретного ДЦ) |
| Расположение ДЦ | Близко к источникам стабильного питания (АЭС, ГЭС) и основным магистралям связи. Учитывается географическое распределение пользователей (Москва, Екатеринбург, Новосибирск, Минск, Алматы, Кишинев). Наличие развитой инфраструктуры поблизости для быстрого реагирования. Сетевые фильтры для предотвращения перегрузок. |
| Внутреннее расположение | Четкое зонирование: серверные стойки, ряды. |
| Защита от перегрева   | Система кондиционирования с разделением на горячие и холодные коридоры, фальш-полы для оптимизации воздушных потоков. |
| Защита от погодных условий/протечек | Фальш-потолок, системы обнаружения протечек. |
| Электропитание   | Резервирование линий питания (N+1 или 2N), использование ИБП (UPS) и дизель-генераторов (ДГУ). |
| Сеть  | Дублирование маршрутизаторов, коммутаторов, сетевых карт (bonding/teaming). Несколько независимых интернет-провайдеров. |
| Память    | Использование SSD (включая NVMe) для баз данных и кэшей (PostgreSQL, Redis, ElasticSearch) для быстрого доступа. HDD/объектное хранилище для медиафайлов (аудио). |
| Отказоустойчивость хранения | RAID (например, RAID 10) для критичных баз данных (PostgreSQL). Репликация данных в NoSQL (Cassandra, ClickHouse) и объектных хранилищах. Регулярное резервное копирование. |

Балансировка:
| Вид | Описание |
| ---   | ---   |
| Глобальная | **GeoDNS** для направления пользователей к ближайшему ДЦ. **BGP Anycast** для стриминговых серверов (stream.music.vk.com) для минимизации задержек и быстрого переключения при сбоях. **CDN** для кеширования статики (cdn.music.vk.com). |
| Локальная | **L7 (Nginx)** для API-запросов (api.music.vk.com) и статики, терминирование SSL. **L4 (HAProxy/Nginx)** для аудиопотоков (stream.music.vk.com). **Kubernetes** для балансировки нагрузки между подами микросервисов и обеспечения отказоустойчивости.  |

Уровень сервисов:
| Вид   | Описание  |
| ---   | ----      |
| Резервирование БД | Стратегии репликации для разных СУБД (описано в п.6): Синхронная/асинхронная репликация **PostgreSQL**, репликация с фактором N в **Cassandra** и **ClickHouse**, репликация **Redis**. Шардирование истории прослушиваний. |
| Graceful shutdown | Корректное завершение работы сервисов при обновлении или остановке: сохранение состояния, завершение обработки текущих запросов, отписка от очередей (**Kafka**). |
| Graceful degradation | При отказе некритичных сервисов (например, рекомендаций или аналитики) основные функции (поиск, прослушивание) продолжают работать. Пользователь может получить стандартный набор треков вместо персонализированных. |
| Circuit breaker | Использование в межсервисных взаимодействиях (например, через **Kubernetes Service Mesh** или библиотеки в коде). Предотвращает каскадные сбои при недоступности одного из микросервисов.|
| Логирование | Централизованный сбор логов ( **ELK Stack** - Elasticsearch, Logstash, Kibana) |
| Мониторинг  | Сбор метрик с серверов и приложений (**Prometheus**). Визуализация (**Grafana**). Мониторинг ключевых бизнес-метрик (количество активных стримов, RPS API, процент ошибок) и системных метрик (CPU, RAM, сеть, диск).|
...
#### Удельное потребление ресурсов на единицу нагрузки

| Технология | Характер сервиса | RPS | RAM |
|------------|------------------|-----|-----|
| Go         | API Gateway      | 1000 | 100 MB |
| Go         | Auth Service     | 500  | 50 MB  |
| Go         | Search Service   | 500  | 50 MB  |
| Go         | Playlist Service | 500  | 50 MB  |
| Go         | Streaming Service| 100  | 100 MB |
| Go         | Recommendation Service | 100 | 100 MB |
| Go         | Analytics Service | 100 | 100 MB |
| Nginx      | SSL   | 500  | 10 MB  |
| HAProxy    | L4 балансировка  | 1000 | 10 MB  |

**Расчеты:**
- **API Gateway**: 7.2k RPS / 1000 RPS на сервер = 7.2 серверов (округляем до 8)
- **Auth Service**: 232 RPS / 500 RPS на сервер = 0.46 серверов (округляем до 1)
- **Search Service**: 1,160 RPS / 500 RPS на сервер = 2.32 серверов (округляем до 3)
- **Playlist Service**: 232 RPS / 500 RPS на сервер = 0.46 серверов (округляем до 1)
- **Streaming Service**: 128 Gbit/s / 300 Mbit/s на сервер = 437 серверов (с запасом 874)
- **Recommendation Service**: 300 RPS / 100 RPS на сервер = 3 сервера
- **Analytics Service**: 1,388 RPS / 100 RPS на сервер = 13.88 серверов (округляем до 14)
- **HAProxy**: 128 Gbit/s / 300 Mbit/s на сервер = 437 серверов (с запасом 874)
- **Nginx**: 7.2k RPS / 500 RPS на сервер = 14.4 серверов (округляем до 15)

#### Расчёт ресурсов для сервисов

| Сервис | Целевая пиковая нагрузка | CPU | RAM | Net |
|--------|--------------------------|-----|-----|-----|
| API Gateway | 7.2k RPS | 72 | 720 MB | 0.06 Gbit/s |
| Auth Service | 232 RPS | 2.3 | 116 MB | 0.002 Gbit/s |
| Search Service | 1,160 RPS | 11.6 | 580 MB | 0.01 Gbit/s |
| Playlist Service | 232 RPS | 2.3 | 116 MB | 0.002 Gbit/s |
| Streaming Service | 128 Gbit/s | 1280 | 12.8 GB | 128 Gbit/s |
| Recommendation Service | 300 RPS | 3 | 300 MB | 0.003 Gbit/s |
| Analytics Service | 1,388 RPS | 13.9 | 1.4 GB | 0.014 Gbit/s |
| HAProxy | 128 Gbit/s | 128 | 1.28 GB | 128 Gbit/s |
| Nginx | 7.2k RPS | 14.4 | 72 MB | 0.06 Gbit/s |

**Расчеты:**
- **API Gateway**: 
  - CPU: 7.2k RPS / 100 RPS на ядро = 72 ядра
  - RAM: 7.2k RPS × 100 MB = 720 MB
  - Net: 7.2k RPS × 1 KB × 8 бит = 0.06 Gbit/s

- **Auth Service**:
  - CPU: 232 RPS / 100 RPS на ядро = 2.3 ядра
  - RAM: 232 RPS × 50 MB = 116 MB
  - Net: 232 RPS × 1 KB × 8 бит = 0.002 Gbit/s

- **Search Service**:
  - CPU: 1,160 RPS / 100 RPS на ядро = 11.6 ядер
  - RAM: 1,160 RPS × 50 MB = 580 MB
  - Net: 1,160 RPS × 1 KB × 8 бит = 0.01 Gbit/s
...
- **Streaming Service**:
  - CPU: 128 Gbit/s / 0.1 Gbit/s на ядро = 1280 ядер
  - RAM: 128 Gbit/s × 100 MB = 12.8 GB
  - Net: 128 Gbit/s

- **Recommendation Service**:
  - CPU: 300 RPS / 100 RPS на ядро = 3 ядра
  - RAM: 300 RPS × 100 MB = 300 MB
  - Net: 300 RPS × 1 KB × 8 бит = 0.003 Gbit/s

- **Analytics Service**:
  - CPU: 1,388 RPS / 100 RPS на ядро = 13.9 ядер
  - RAM: 1,388 RPS × 100 MB = 1.4 GB
  - Net: 1,388 RPS × 1 KB × 8 бит = 0.014 Gbit/s

- **HAProxy**:
  - CPU: 128 Gbit/s / 1 Gbit/s на ядро = 128 ядер
  - RAM: 128 Gbit/s × 10 MB = 1.28 GB
  - Net: 128 Gbit/s

- **Nginx**:
  - CPU: 7.2k RPS / 500 RPS на ядро = 14.4 ядра
  - RAM: 7.2k RPS × 10 MB = 72 MB
  - Net: 7.2k RPS × 1 KB × 8 бит = 0.06 Gbit/s

### Выбор модели развертывания

#### Конфигурации серверов для Kubernetes

| Сервис | CPU/r | CPU/l | RAM/r | RAM/l | Cnt | Пояснение |
|--------|-------|-------|-------|-------|-----|-----------|
| API Gateway | 8 | 8 | 8 GB | 32 GB | 9 | Нужно 9 серверов. На каждом: 8 ядер CPU (максимум 8), 8 GB RAM (максимум 32 GB). Это серверы для обработки API-запросов. |
| Auth Service | 4 | 4 | 4 GB | 8 GB | 1 | Нужен 1 сервер. На нем: 4 ядра CPU (максимум 4), 4 GB RAM (максимум 8 GB). Это сервер аутентификации. |
| Search Service | 8 | 8 | 8 GB | 16 GB | 2 | Нужно 2 сервера. На каждом: 8 ядер CPU (максимум 8), 8 GB RAM (максимум 16 GB). Это серверы поиска. |
| Playlist Service | 4 | 4 | 4 GB | 8 GB | 1 | Нужен 1 сервер. На нем: 4 ядра CPU (максимум 4), 4 GB RAM (максимум 8 GB). Это сервер плейлистов. |
| Streaming Service | 16 | 16 | 64 GB | 128 GB | 80 | Нужно 80 серверов. На каждом: 16 ядер CPU (максимум 16), 64 GB RAM (максимум 128 GB). Это серверы стриминга. |
| Recommendation Service | 4 | 4 | 4 GB | 8 GB | 1 | Нужен 1 сервер. На нем: 4 ядра CPU (максимум 4), 4 GB RAM (максимум 8 GB). Это сервер рекомендаций. |
| Analytics Service | 8 | 8 | 8 GB | 16 GB | 2 | Нужно 2 сервера. На каждом: 8 ядер CPU (максимум 8), 8 GB RAM (максимум 16 GB). Это серверы аналитики. |
| HAProxy | 8 | 8 | 8 GB | 16 GB | 16 | Нужно 16 серверов. На каждом: 8 ядер CPU (максимум 8), 8 GB RAM (максимум 16 GB). Это балансировщики нагрузки. |
| Nginx | 8 | 8 | 8 GB | 16 GB | 2 | Нужно 2 сервера. На каждом: 8 ядер CPU (максимум 8), 8 GB RAM (максимум 16 GB). Это веб-серверы. |

#### Конфигурации серверов для баз данных

| Сервис | Хостинг | Конфигурация | Cores | Cnt | RAM | Storage | Пояснение |
|--------|---------|--------------|-------|-----|-----|---------|-----------|
| PostgreSQL | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 6 | 512 GB | 24 TB | Основная реляционная БД. 6 серверов (3 реплики × 2 шарда). На каждом: 64 ядра CPU, 512 GB RAM, 24 TB хранилища. |
| Cassandra | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 12 | 384 GB | 48 TB | NoSQL БД для истории прослушиваний. 12 серверов (6 реплик × 2 шарда). На каждом: 64 ядра CPU, 384 GB RAM, 48 TB хранилища. |
| ClickHouse | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 8 | 256 GB | 32 TB | Аналитическая БД. 8 серверов (4 реплики × 2 шарда). На каждом: 64 ядра CPU, 256 GB RAM, 32 TB хранилища. |
| Redis | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 12 | 384 GB | 48 TB | Кэш-сервер. 12 серверов (6 реплик × 2 шарда). На каждом: 64 ядра CPU, 384 GB RAM, 48 TB хранилища. |

#### Конфигурации серверов для мониторинга и логирования

| Сервис | Хостинг | Конфигурация | Cores | Cnt | RAM | Storage | Пояснение |
|--------|---------|--------------|-------|-----|-----|---------|-----------|
| Prometheus + Grafana | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 4 | 128 GB | 16 TB | Система мониторинга. 4 сервера (2 реплики × 2 шарда). На каждом: 64 ядра CPU, 128 GB RAM, 16 TB хранилища. |
| ELK Stack | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 64 | 8 | 256 GB | 32 TB | Система логирования. 8 серверов (4 реплики × 2 шарда). На каждом: 64 ядра CPU, 256 GB RAM, 32 TB хранилища. |

### Итоговый подсчёт ресурсов


| Тип серверов | Количество | vCPU | RAM (GB) | Storage (TB) | Пояснение |
|--------------|------------|------|----------|--------------|-----------|
| API серверы | 24 | 192 | 768 | 2.4 | Серверы для обработки API-запросов. 24 сервера, всего 192 ядра CPU, 768 GB RAM, 2.4 TB хранилища. |
| Стриминговые серверы | 874 | 13,984 | 55,936 | 874 | Серверы для стриминга музыки. 874 сервера, всего 13,984 ядер CPU, 55,936 GB RAM, 874 TB хранилища. |
| PostgreSQL | 6 | 384 | 3,072 | 144 | Основная БД. 6 серверов, всего 384 ядра CPU, 3,072 GB RAM, 144 TB хранилища. |
| Cassandra | 12 | 768 | 4,608 | 576 | NoSQL БД. 12 серверов, всего 768 ядер CPU, 4,608 GB RAM, 576 TB хранилища. |
| ClickHouse | 8 | 512 | 2,048 | 256 | Аналитическая БД. 8 серверов, всего 512 ядер CPU, 2,048 GB RAM, 256 TB хранилища. |
| Redis | 12 | 768 | 4,608 | 576 | Кэш-серверы. 12 серверов, всего 768 ядер CPU, 4,608 GB RAM, 576 TB хранилища. |
| HAProxy | 8 | 64 | 128 | 0.8 | Балансировщики нагрузки. 8 серверов, всего 64 ядра CPU, 128 GB RAM, 0.8 TB хранилища. |
| Nginx | 16 | 128 | 256 | 1.6 | Веб-серверы. 16 серверов, всего 128 ядер CPU, 256 GB RAM, 1.6 TB хранилища. |
| Prometheus + Grafana | 4 | 256 | 512 | 64 | Серверы мониторинга. 4 сервера, всего 256 ядер CPU, 512 GB RAM, 64 TB хранилища. |
| ELK Stack | 8 | 512 | 2,048 | 256 | Серверы логирования. 8 серверов, всего 512 ядер CPU, 2,048 GB RAM, 256 TB хранилища. |
| **Итого** | 972 | 15,152 | 60,544 | 952.2 | Общее количество ресурсов для всего проекта. |

**Расчеты:**
- **API серверы**:
  - vCPU: 24 сервера × 8 ядер = 192 vCPU
  - RAM: 24 сервера × 32 GB = 768 GB
  - Storage: 24 сервера × 100 GB = 2.4 TB

- **Стриминговые серверы**:
  - vCPU: 874 сервера × 16 ядер = 13,984 vCPU
  - RAM: 874 сервера × 64 GB = 55,936 GB
  - Storage: 874 сервера × 1 TB = 874 TB

- **PostgreSQL**:
  - vCPU: 6 серверов × 64 ядра = 384 vCPU
  - RAM: 6 серверов × 512 GB = 3,072 GB
  - Storage: 6 серверов × 24 TB = 144 TB

- **Cassandra**:
  - vCPU: 12 серверов × 64 ядра = 768 vCPU
  - RAM: 12 серверов × 384 GB = 4,608 GB
  - Storage: 12 серверов × 48 TB = 576 TB

- **ClickHouse**:
  - vCPU: 8 серверов × 64 ядер = 512 vCPU
  - RAM: 8 серверов × 256 GB = 2,048 GB
  - Storage: 8 серверов × 32 TB = 256 TB
...
- **HAProxy**:
  - vCPU: 8 серверов × 8 ядер = 64 vCPU
  - RAM: 8 серверов × 16 GB = 128 GB
  - Storage: 8 серверов × 100 GB = 0.8 TB

- **Nginx**:
  - vCPU: 16 серверов × 8 ядер = 128 vCPU
  - RAM: 16 серверов × 16 GB = 256 GB
  - Storage: 16 серверов × 100 GB = 1.6 TB

- **Prometheus + Grafana**:
  - vCPU: 4 сервера × 64 ядра = 256 vCPU
  - RAM: 4 сервера × 128 GB = 512 GB
  - Storage: 4 сервера × 16 TB = 64 TB

- **ELK Stack**:
  - vCPU: 8 серверов × 64 ядра = 512 vCPU
  - RAM: 8 серверов × 256 GB = 2,048 GB
  - Storage: 8 серверов × 32 TB = 256 TB
