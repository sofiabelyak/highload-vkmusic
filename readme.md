...
| Таблица                | Запись (QPS) | Чтение (QPS) | Описание операций и расчет                                                                 |
|------------------------|--------------|--------------|-------------------------------------------------------------------------------------------|
| **users**              | 116          | 1,160        | **Запись**: Регистрация, обновление данных пользователя. **Чтение**: Аутентификация, получение профиля. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `10M DAU × 10 действий / 86,400` (чтение). |
| **search_queries**     | 580          | 5,000        | **Запись**: Сохранение поисковых запросов. **Чтение**: Поиск треков, плейлистов, артистов. **Расчет**: `10M DAU × 5 запросов / 86,400` (запись), `10M DAU × 5 запросов / 86,400` (чтение). |
| **listening_history**  | 1,388        | 12,000       | **Запись**: Запись истории прослушиваний. **Чтение**: Получение истории прослушиваний пользователя. **Расчет**: `10M DAU × 12 треков / 86,400` (запись), `10M DAU × 12 треков / 86,400` (чтение). |
| **playlist_tracks**    | 116          | 1,160        | **Запись**: Добавление/удаление треков в плейлист. **Чтение**: Получение треков из плейлиста. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `10M DAU × 10 действий / 86,400` (чтение). |
| **playlists**          | 16           | 160          | **Запись**: Создание/редактирование плейлистов. **Чтение**: Получение списка плейлистов пользователя. **Расчет**: `10M DAU × 0.14 действия / 86,400` (запись), `10M DAU × 0.14 действий / 86,400` (чтение). |
| **recommendations**    | 116          | 1,160        | **Запись**: Генерация рекомендаций. **Чтение**: Получение рекомендаций для пользователя. **Расчет**: `10M DAU × 1 рекомендация / 86,400` (запись), `10M DAU × 10 рекомендаций / 86,400` (чтение). |
| **subscriptions**      | 12           | 120          | **Запись**: Оформление/обновление подписки. **Чтение**: Проверка статуса подписки. **Расчет**: `10M DAU × 0.1 действия / 86,400` (запись), `10M DAU × 0.1 действий / 86,400` (чтение). |
| **payments**           | 12           | 120          | **Запись**: Обработка платежей. **Чтение**: Получение истории платежей пользователя. **Расчет**: `10M DAU × 0.1 действия / 86,400` (запись), `10M DAU × 0.1 действий / 86,400` (чтение). |
| **tracks**             | 0            | 10,000       | **Чтение**: Получение данных о треках (метаданные, файлы). **Расчет**: `10M DAU × 12 треков / 86,400` (чтение). Запись не требуется, так как треки добавляются администраторами. |
| **artists**            | 0            | 1,000        | **Чтение**: Получение данных об артистах. **Расчет**: `10M DAU × 0.1 запросов / 86,400` (чтение). Запись не требуется, так как артисты добавляются администраторами. |
| **albums**             | 0            | 2,000        | **Чтение**: Получение данных об альбомах. **Расчет**: `10M DAU × 0.2 запросов / 86,400` (чтение). Запись не требуется, так как альбомы добавляются администраторами. |
| **genres**             | 0            | 500          | **Чтение**: Получение списка жанров. **Расчет**: `10M DAU × 0.05 запросов / 86,400` (чтение). Запись не требуется, так как жанры добавляются администраторами. |
| **track_genres**       | 0            | 1,000        | **Чтение**: Получение жанров для треков. **Расчет**: `10M DAU × 0.1 запросов / 86,400` (чтение). Запись не требуется, так как связи добавляются администраторами. |
| **favorites**          | 116          | 580          | **Запись**: Добавление/удаление треков в избранное. **Чтение**: Получение списка избранных треков. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `10M DAU × 5 действий / 86,400` (чтение). |
