...
**Расчеты:**
- **API Gateway**: 7200 RPS / 1000 RPS на сервер = 8 серверов × 2 × 1.5 = **24 сервера**
...
- **Streaming Service**: 128 Gbit/s / 300 Mbit/s на сервер = 427 серверов × 2 × 1.2 = **1025 серверов**
...
|--------|--------------------------|-----|-----|-----|
| API Gateway | 7200 RPS | 8 | 0.8 GB | 0.6 Gbit/s |
...
| Streaming Service | 128 Gbit/s | 2 | 0.2 GB | 128 Gbit/s |
...
**Расчеты CPU:**
- **API Gateway**: 7200 RPS / 1000 (легкая логика) = 7.2 CPU = 8
...
- **Streaming Service**: 128 / 100 (средняя логика) = 2 CPU
...
|--------|---------|--------------|-------|-----|-----|---------|
| PostgreSQL | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 1 | 8 | 512 GB | 24 TB |
| Cassandra | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 2 | 16 | 384 GB | 48 TB |
| ClickHouse | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 2 | 12 | 256 GB | 32 TB |
| Redis | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 2 | 16 | 384 GB | 48 TB |

#### Конфигурации серверов для мониторинга и логирования

| Сервис | Хостинг | Конфигурация | CPU | Cnt | RAM | Storage |
|--------|---------|--------------|-------|-----|-----|---------|
| Prometheus + Grafana | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 8 | 4 | 128 GB | 16 TB |
| ELK Stack | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 8 | 8 | 256 GB | 32 TB |

**Расчеты CPU:**
- **PostgreSQL**: 348 RPS / 1000 = 0.348 ≈ 1 CPU
- **Cassandra**: 1,388 RPS / 1000 = 1.388 ≈ 2 CPU
- **ClickHouse**: 1,388 RPS / 1000 = 1.388 ≈ 2 CPU
- **Redis**: 1,388 RPS / 1000 = 1.388 ≈ 2 CPU
- **Prometheus + Grafana**: 7,200 RPS / 1000 = 7.2 ≈ 8 CPU
- **ELK Stack**: 7,200 RPS / 1000 = 7.2 ≈ 8 CPU

### Итоговый подсчёт ресурсов

| Тип серверов | Количество | Физические CPU | RAM (GB) | Storage (TB) |
|--------------|------------|----------------|----------|--------------|
| API серверы | 471 | 15 | 3,768 | 47.1 |
| Стриминговые серверы | 1025 | 65 | 16,400 | 1025 |
| PostgreSQL | 8 | 8 | 4,096 | 192 |
| Cassandra | 16 | 32 | 6,144 | 768 |
| ClickHouse | 12 | 24 | 3,072 | 384 |
| Redis | 16 | 32 | 6,144 | 768 |
| HAProxy | 1025 | 65 | 8,200 | 102.5 |
| Nginx | 6 | 48 | 12 | 0.6 |
| Prometheus + Grafana | 4 | 32 | 512 | 64 |
| ELK Stack | 8 | 64 | 2,048 | 256 |
| **Итого** | 2,591 | 385 | 50,296 | 3,607.2 |

**Расчеты:**
- **API серверы**: 471 серверов × 1 CPU = 15 физических CPU
- **Стриминговые серверы**: 1025 серверов × 1 CPU = 65 физических CPU
- **PostgreSQL**: 8 серверов × 1 CPU = 8 физических CPU
- **Cassandra**: 16 серверов × 2 CPU = 32 физических CPU
- **ClickHouse**: 12 серверов × 2 CPU = 24 физических CPU
- **Redis**: 16 серверов × 2 CPU = 32 физических CPU
- **HAProxy**: 1025 серверов × 1 CPU = 65 физических CPU
- **Nginx**: 6 серверов × 8 CPU = 48 физических CPU
- **Prometheus + Grafana**: 4 сервера × 8 CPU = 32 физических CPU
- **ELK Stack**: 8 серверов × 8 CPU = 64 физических CPU
