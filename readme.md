...
2. **Микросервисы**:
   - Каждый сервис обрабатывает свой тип запросов:
     ...
     - Track Service: сервис для треков и стриминга
...
| Вид | Описание |
| ---   | ---   |
| Глобальная | **GeoDNS** для направления пользователей к ближайшему ДЦ. **BGP Anycast** для стриминговых серверов (stream.music.vk.com) для минимизации задержек и быстрого переключения при сбоях. |
...
| Go         | Auth Service     | 1000  | 500 MB  |
| Go         | Search Service   | 10  | 100 MB  |
| Go         | Playlist Service | 100  | 100 MB  |
| Go         | Track Service| 100  | 100 MB |
| Go         | Recommendation Service | 100 | 100 MB |
| Go         | Analytics Service | 100 | 100 MB |
| Nginx      | SSL   | 5000  | 10 MB  |
| HAProxy    | L4 балансировка  | 1000 | 10 MB  |

**Практические допущения:**  
В реальных условиях серверы не нагружают на 100% — оптимальной считается загрузка не более 50% по CPU и RAM. Поэтому расчетное количество серверов увеличивается в 2 раза.

**Формула:**  
Количество серверов = (расчет на 100% загрузки) × 2 * 1.5

**Расчеты:**
- **API Gateway**: (7200 RPS (Auth) + 1,160 RPS (Search) + 232 RPS (Playlist) + 2,084 RPS (Track) + 300 RPS (Recommendation)) = 10,976 RPS / 1000 RPS на сервер = 11 серверов × 2 × 1.5 = **33 сервера**
- **Auth Service**: 7200 RPS / 1000 RPS на сервер = 8 серверов × 2 × 1.5 = **24 сервера**
- **Search Service**: 1,160 RPS / 10 RPS на сервер = 116 серверов × 2 × 1.5 = **348 серверов**
- **Playlist Service**: 232 RPS / 100 RPS на сервер = 3 сервера × 2 × 1.5 = **9 серверов**
- **Track Service**: 128 Gbit/s / 300 Mbit/s на сервер = 427 серверов × 2 × 1.2 = **1025 серверов**
- **Recommendation Service**: 300 RPS / 100 RPS на сервер = 3 сервера × 2 × 1.5 = **9 серверов**
- **Analytics Service**: 1,388 RPS / 100 RPS на сервер = 14 серверов × 2 × 1.5 = **42 сервера**
- **HAProxy**: 128 Gbit/s / 300 Mbit/s на сервер = 427 серверов × 2 × 1.2 = **1025 серверов**
- **Nginx**: 7200 RPS / 5000 RPS на сервер = 2 сервера × 2 × 1.5 = **6 серверов**

#### Расчёт ресурсов для сервисов

| Сервис | Целевая пиковая нагрузка | CPU | RAM | Net |
|--------|--------------------------|-----|-----|-----|
| API Gateway | 10,976 RPS | 11 | 1.1 GB | 0.86 Gbit/s |
| Auth Service | 7200 RPS | 72 | 7.2 GB | 0.6 Gbit/s |
| Search Service | 1,160 RPS | 116 | 11.6 GB | 0.01 Gbit/s |
| Playlist Service | 232 RPS | 23.2 | 2.32 GB | 0.002 Gbit/s |
| Track Service | 128 Gbit/s | 12 | 7.2 GB | 128 Gbit/s |
| Recommendation Service | 300 RPS | 30 | 3 GB | 0.003 Gbit/s |
| Analytics Service | 1,388 RPS | 138.8 | 13.88 GB | 0.014 Gbit/s |
| HAProxy | 128 Gbit/s | 128 | 12.8 GB | 128 Gbit/s |
| Nginx | 7200 RPS | 8 | 0.8 GB | 0.6 Gbit/s |

**Расчеты CPU:**
- **API Gateway**: 10,976 RPS / 1000 (легкая логика) = 10.976 CPU ≈ 11 CPU
- **Auth Service**: 7200 RPS / 100 (средняя логика) = 72 CPU
- **Search Service**: 1,160 RPS / 10 (сложная логика) = 116 CPU
- **Playlist Service**: 232 RPS / 10 (сложная логика) = 23.2 CPU
- **Track Service**: 128 / 100 (средняя логика) = 1.28 ≈ 2 CPU (для стриминга) + 10 CPU (для управления треками и взаимодействия с облачным хранилищем) = 12 CPU
- **Recommendation Service**: 300 RPS / 10 (сложная логика) = 30 CPU
- **Analytics Service**: 1,388 RPS / 10 (сложная логика) = 138.8 CPU
- **HAProxy**: 128 CPU (прямая нагрузка)
- **Nginx**: 7200 RPS / 1000 (легкая логика) = 7.2 CPU = 8

**Расчеты RAM:**
- **API Gateway**: 11 CPU × 100 MB = 1.1 GB
- **Auth Service**: 72 CPU × 100 MB = 7.2 GB
- **Search Service**: 116 CPU × 100 MB = 11.6 GB
- **Playlist Service**: 23.2 CPU × 100 MB = 2.32 GB
- **Track Service**: 12 CPU × 100 MB = 1.2 GB + 6 GB (для кэширования популярных треков и метаданных из облачного хранилища) = 7.2 GB
- **Recommendation Service**: 30 CPU × 100 MB = 3 GB
- **Analytics Service**: 138.8 CPU × 100 MB = 13.88 GB
- **HAProxy**: 128 CPU × 100 MB = 12.8 GB
- **Nginx**: 8 CPU × 100 MB = 0.8 GB

#### Конфигурации серверов для баз данных

| Сервис | Хостинг | Конфигурация | CPU | Cnt | RAM | Storage |
|--------|---------|--------------|-------|-----|-----|---------|
| PostgreSQL | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 4 | 8 | 512 GB | 24 TB |
| Cassandra | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 14 | 16 | 384 GB | 48 TB |
| ClickHouse | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 14 | 12 | 256 GB | 32 TB |
| Redis | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 14 | 16 | 384 GB | 48 TB |

#### Конфигурации серверов для мониторинга и логирования

| Сервис | Хостинг | Конфигурация | CPU | Cnt | RAM | Storage |
|--------|---------|--------------|-------|-----|-----|---------|
| Prometheus + Grafana | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 72 | 4 | 128 GB | 16 TB |
| ELK Stack | own | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 72 | 8 | 256 GB | 32 TB |

**Расчеты CPU:**
- **PostgreSQL**: 348 RPS / 100 = 3.48 ≈ 4 CPU
- **Cassandra**: 1,388 RPS / 100 = 13.88 ≈ 14 CPU
- **ClickHouse**: 1,388 RPS / 100 = 13.88 ≈ 14 CPU
- **Redis**: 1,388 RPS / 100 = 13.88 ≈ 14 CPU
- **Prometheus + Grafana**: 7,200 RPS / 100 = 72 CPU
- **ELK Stack**: 7,200 RPS / 100 = 72 CPU

### Итоговый подсчёт ресурсов

| Тип серверов | Количество | Физические CPU | RAM (GB) | Storage (TB) |
|--------------|------------|----------------|----------|--------------|
| API серверы | 480 | 18 | 3,840 | 48.0 |
| Стриминговые серверы | 1025 | 65 | 7,380 | 1025 |
| PostgreSQL | 8 | 32 | 4,096 | 192 |
| Cassandra | 16 | 224 | 6,144 | 768 |
| ClickHouse | 12 | 168 | 3,072 | 384 |
| Redis | 16 | 224 | 6,144 | 768 |
| HAProxy | 1025 | 65 | 8,200 | 102.5 |
| Nginx | 6 | 48 | 12 | 0.6 |
| Prometheus + Grafana | 4 | 288 | 512 | 64 |
| ELK Stack | 8 | 576 | 2,048 | 256 |
| **Итого** | 2,600 | 1,708 | 41,348 | 3,608.1 |

**Расчеты:**
- **API серверы**: (33 + 24 + 348 + 9 + 9 + 42 + 15) = 480 серверов × 1 CPU = 18 физических CPU
- **Стриминговые серверы**: 1025 серверов × 1 CPU = 65 физических CPU
- **PostgreSQL**: 8 серверов × 4 CPU = 32 физических CPU
- **Cassandra**: 16 серверов × 14 CPU = 224 физических CPU
- **ClickHouse**: 12 серверов × 14 CPU = 168 физических CPU
- **Redis**: 16 серверов × 14 CPU = 224 физических CPU
- **HAProxy**: 1025 серверов × 1 CPU = 65 физических CPU
- **Nginx**: 6 серверов × 8 CPU = 48 физических CPU
- **Prometheus + Grafana**: 4 сервера × 72 CPU = 288 физических CPU
- **ELK Stack**: 8 серверов × 72 CPU = 576 физических CPU
