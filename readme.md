...
|------------------------|--------------|--------------|-------------------------------------------------------------------------------------------|
| **users**              | 116          | 580          | **Запись**: Регистрация, обновление данных пользователя. **Чтение**: Аутентификация, получение профиля. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `116 × 5` (чтение). |
| **search_queries**     | 580          | 2,900        | **Запись**: Сохранение поисковых запросов. **Чтение**: Поиск треков, плейлистов, артистов. **Расчет**: `10M DAU × 5 запросов / 86,400` (запись), `580 × 5` (чтение). |
| **listening_history**  | 1,388        | 6,940        | **Запись**: Запись истории прослушиваний. **Чтение**: Получение истории прослушиваний пользователя. **Расчет**: `10M DAU × 12 треков / 86,400` (запись), `1,388 × 5` (чтение). |
| **playlist_tracks**    | 116          | 580          | **Запись**: Добавление/удаление треков в плейлист. **Чтение**: Получение треков из плейлиста. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `116 × 5` (чтение). |
| **playlists**          | 16           | 80           | **Запись**: Создание/редактирование плейлистов. **Чтение**: Получение списка плейлистов пользователя. **Расчет**: `10M DAU × 0.14 действия / 86,400` (запись), `16 × 5` (чтение). |
| **recommendations**    | 116          | 580          | **Запись**: Генерация рекомендаций. **Чтение**: Получение рекомендаций для пользователя. **Расчет**: `10M DAU × 1 рекомендация / 86,400` (запись), `116 × 5` (чтение). |
| **subscriptions**      | 12           | 60           | **Запись**: Оформление/обновление подписки. **Чтение**: Проверка статуса подписки. **Расчет**: `10M DAU × 0.1 действия / 86,400` (запись), `12 × 5` (чтение). |
| **payments**           | 12           | 60           | **Запись**: Обработка платежей. **Чтение**: Получение истории платежей пользователя. **Расчет**: `10M DAU × 0.1 действия / 86,400` (запись), `12 × 5` (чтение). |
...
| **favorites**          | 116          | 580          | **Запись**: Добавление/удаление треков в избранное. **Чтение**: Получение списка избранных треков. **Расчет**: `10M DAU × 1 действие / 86,400` (запись), `116 × 5` (чтение). |
